...
namespace Longmao.Web.Sites
{
    public partial class manage : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("login.aspx?url=manage.aspx");
            }
            else
            {
            ...
            try
            {
                string sql = "select * from [tb_site] order by site_createtime desc ";
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        strSiteInfo += "<a href=\"javascript:manage_control('" + dt.Rows[i]["site_id"].ToString() + "');\" data-id=\"" + dt.Rows[i]["site_id"].ToString() + "\" class=\"config\"></a>";
